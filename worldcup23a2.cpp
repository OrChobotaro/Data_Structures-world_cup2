...
//    per = per * player->getSpirit();
    if(player->getUp()){
        per = player->getUp()->getCalcPartialSpirit();
    }
...
StatusType world_cup_t::buy_team(int teamId1, int teamId2)
{
    ...
    if (numPlayersBuyerTeam >= 0  &&  numPlayersBoughtTeam == 0) {
        ...
        AbilityDataTeam oldAbilityBigTeam(buyerTeam->getKey().getTeamID(), buyerTeam->getKey().getTeamAbility());
        ...
        AbilityDataTeam newAbilityBigTeam(buyerTeam->m_key.getTeamID(), buyerTeam->m_key.getTeamAbility());
...
Node<TeamData>* world_cup_t::findTeam(PlayerData *player) {
  ...
  assert(player);
    ...
    permutation_t multiplePartialSpirit = calcMultiple(player); /// changed
...
//    multiplePartialSpirit = multiplePartialSpirit * (reversedRoot->getCalcPartialSpirit()).inv();
    ...
    while (temp != reversedRoot) {
        ...
        temp->setCalcPartialSpirit(reversedRoot->getCalcPartialSpirit().inv() * multiplePartialSpirit * toDivide); /// changed

        toSubtract += tempTotalGamesPlayed;
        toDivide = toDivide * temp->getCalcPartialSpirit().inv(); /// changed
...
permutation_t world_cup_t::calcMultiple(PlayerData* player){

    if(!player)
        return permutation_t::neutral();

    return calcMultiple(player->getUp()) * player->getCalcPartialSpirit();
...
void world_cup_t::unionPlayerToRegularTeam(PlayerData* playerNode, Node<TeamData>* teamNode) {
    ...
    permutation_t newCalcPartialSpirit = (reversedRoot->getCalcPartialSpirit().inv() * newTeamSpirit); /// changed todo: change to calcPartialSpirit
...
void world_cup_t::unionBigBuyerTeamToSmallTeam(Node<TeamData> *bigTeamNode, Node<TeamData> *smallTeamNode) {
    ...
    permutation_t reversedRootBigTeamSpirit = reversedRootBigTeam->getCalcPartialSpirit(); /// changed
    permutation_t reversedRootSmallTeamSpirit = reversedRootSmallTeam->getCalcPartialSpirit(); /// changed

    reversedRootSmallTeam->setCalcPartialSpirit(reversedRootBigTeamSpirit.inv() * bigTeamSpirit
                                                * reversedRootSmallTeamSpirit); /// changed
    ...
    AbilityDataTeam newAbilityBigTeam(bigTeamNode->m_key.getTeamID(), bigTeamNode->m_key.getTeamAbility());
    m_rankAbilityTree->insert(newAbilityBigTeam);
...
void world_cup_t::unionSmallBuyerTeamToBigTeam(Node<TeamData> *bigTeamNode, Node<TeamData> **smallTeamNode) {
    ...
    int newPoints = bigTeamNode->getKey().getTeamPoints() + (*smallTeamNode)->getKey().getTeamPoints();
    int newAbility = bigTeamNode->getKey().getTeamAbility() + (*smallTeamNode)->getKey().getTeamAbility();
    permutation_t newSpirit = (*smallTeamNode)->getKey().getTeamSpirit() * bigTeamNode->getKey().getTeamSpirit(); /// changed
    ...
    permutation_t newCalcPartialSpiritBigTeam =  (*smallTeamNode)->getKey().getTeamSpirit() *
            reversedRootBigTeam->getCalcPartialSpirit(); /// changed
    reversedRootBigTeam->setCalcPartialSpirit(newCalcPartialSpiritBigTeam);

    permutation_t newCalcPartialSpiritSmallTeam =  newCalcPartialSpiritBigTeam.inv()
                                                * reversedRootSmallTeam->getCalcPartialSpirit(); /// changed
